...
function Signup() {
  ...
  const roleRef = useRef(); // Add a reference for role selection
  ...
  async function handleSubmit(e) {
    ...
    try {
      setError('');
      setLoading(true);
      await signup(emailRef.current.value, passwordRef.current.value, roleRef.current.value); // Pass the role
      navigate('/');
    } catch (error) {
      switch (error.code) {
